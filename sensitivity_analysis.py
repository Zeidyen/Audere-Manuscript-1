...
prof["care_linkage_hiv_testing_ts"]  = to_sast(prof.apply(
    lambda r: _get_v(r, "care_linkage_hiv_testing", "last_updated_source", "timestamp"), axis=1))
...
s5_pids = set(prof[prof["care_linkage_hiv_testing_val"].notna()
    & ~prof["care_linkage_hiv_testing_val"].isin(["unspecified","clientUnknown",""])
    & (prof["care_linkage_hiv_testing_ts"] >= START)
    & (prof["care_linkage_hiv_testing_ts"] <= END)]["patient_id"].unique()) & all_pids
